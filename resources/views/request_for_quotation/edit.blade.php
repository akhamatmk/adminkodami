...
@section('content')
	...
	<div class="row bg-title">
       ...
       <div class="col-lg-9 col-sm-8 col-md-8 col-xs-12">
			<button class="right-side-toggle waves-effect waves-light btn-info btn-circle pull-right m-l-20"><i class="ti-settings text-white"></i></button>
				<a href="{{ URL('requestForQuotation') }}" class="btn btn-success btn-sm pull-right m-l-20 hidden-xs hidden-sm waves-effect waves-light" target="_top"> <i class="fa fa-plus"></i> Back Request For Quotation</a>
				<ol class="breadcrumb">
	...
	<div class="row">
	   	<div class="col-sm-12">
	      	<div class="white-box">
				<h3 class="box-title">Form Edit Request For Qoutation</h3>
				<form class="form-horizontal" method="POST" action="{{ URL("requestForQuotation") }}/{{ $request->id}}/edit">
					@csrf 

					<div class="form-group">
					    <label class="control-label col-sm-3" for="">Case Id:</label>
					    <div class="col-sm-9">
					    	<input type="text" readonly="readonly" value="{{ $request->case_id }}" class="form-control">
					    </div>
					</div>
					...
					<div class="form-group">
					    ...
					    <div class="col-sm-9">
					    	<input type="text" name="document_title" class="form-control" value="{{ $request->document_title }}">					      
					    </div>
					</div>
					    ...
					    <div class="col-sm-9">
					    	<select class="form-control" id="solicitation_type" name="solicitation_type">
				    			...
				    			@foreach($solicitation_type as $key => $value)
				    				@if($key == $request->solicitation_type)
				    					@php ($select = 'selected="selected"')
				    				@else
				    					@php ($select = '')
				    				@endIf
					...
					<div class="form-group">
					    ...
					    <div class="col-sm-9">
					    	<input type="text" name="delivery_date" class="form-control date" value="{{ $request->delivery_date }}">
					    </div>
					</div>
					...
					<div class="form-group">
					    ...
					    <div class="col-sm-9">
					    	<input type="text" name="expiration_date" class="form-control date" value="{{ $request->expired_date }}">
					    </div>
					</div>
					...
					<div class="form-group">
					    ...
					    <div class="col-sm-9">
					    	<textarea class="form-control" name="detail_delivery_addres" id="detail_delivery_addres">{{ $request->detail_delivery_address}}</textarea>
					    </div>
					</div>
					...
					<div class="form-group">
					    ...
					    <div class="col-sm-9">
					    	<input type="text" name="effective_date" class="form-control date" value="{{ $request->effective_date }}">
					    </div>
					</div>
						...
						<tbody id="body_content">
							<?php 
								foreach ($content as $key => $value) {
									?>
										<tr id='body_<?=$key?>'>
			    							<td><select class='form-control' name='content[<?=$key?>][product]'>
			    									<option value='0'>- Silahkan Pulih --</option> 
			    										@foreach($product as $keyproduct => $valueproduct) 
			    											@if($valueproduct->id == $value->kodami_product_id)
										    					@php ($select = 'selected="selected"')
										    				@else
										    					@php ($select = '')
										    				@endIf

			    											<option {{ $select }} value='{{ $value->id }}'>{{ $valueproduct->name }}</option>  
			    										@endForeach
			    								</select>
			    							</td>
			    							<td><input type='number' class='form-control' value="{{ (int) $value['qty'] }}" name='content[<?=$key?>][qty]'></td>
			    							<td><a class='btn btn-danger delete_row' data-id='<?=$key?>'>Hapus Data</a></td>
			    						</tr>
									<?php
								}
							?>
